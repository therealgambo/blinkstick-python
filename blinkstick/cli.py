...
from blinkstick.formatter import IndentedHelpFormatterWithNL
from blinkstick.blinkstick import get_all, get_by_serial
...
def main():
    ...
    parser = OptionParser(usage="usage: %prog [options] [color]", formatter=IndentedHelpFormatterWithNL())
    ...
    group = OptionGroup(parser, "Change color", "These options control the color of the device")
    ...
    group = OptionGroup(parser, "Control animations", "These options will blink, morph or pulse selected color.  ")
    ...
    group.add_option("--morph",
                     dest="morph",
                     action='store_true',
                     help="Morph to specified color (requires --set-color or color set as last argument, "
    ...
    if options.serial is None:
        ...
        sticks = [get_by_serial(options.serial)]
